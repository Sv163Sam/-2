...
public: 
    ...
    broken_line(const unsigned int size, int select_type_line) 
    { 
        _size = size; 
        _p = new point<T>[_size];   
        for (int i = 0; i < _size; i++)
        {   
            ...
            std::cin >> _p[i]._x;
            std::cin >> _p[i]._y;
            if (select_type_line == 1) 
            {
                _p[i]._x = (int)_p[i]._x;
                _p[i]._y = (int)_p[i]._y;
            }
            if (select_type_line == 2)
            {
                _p[i]._x = (double)_p[i]._x;
                _p[i]._y = (double)_p[i]._y;
            }
    ...
    float distance(const ::point<int>& point, const ::point<int>& point1)
    {
        return (float)sqrt(pow((point1._x - point._x), 2) + pow((point1._y - point._y), 2));
    }
    double distance(const ::point<double>& point, const ::point<double>& point1)
    {
       return (double)sqrt(pow((point1._x - point._x), 2) + pow((point1._y - point._y), 2));
    }
    std::complex<double> distance(const point<std::complex<double> >& point1, const point<std::complex<double> >& point2)
    {
        return sqrt(pow((point2._x - point1._x), 2) + pow((point2._y - point1._y), 2));
    }
   
    auto len(const broken_line<T>& line) 
    {
        T var = 0;
        if (line._size == 1)
        {
